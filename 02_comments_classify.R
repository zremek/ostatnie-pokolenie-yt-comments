...
patronizing_prompt <- "Jesteś klasyfikatorem, zawsze odpowiadasz wyłącznie jedną cyfrą: 1 albo 0. Zaklasyfikuj następujący komentarz zamieszczony pod wideo na YouTube jako wyrażający lub nie wyrażający patronizowanie, czyli wypowiedź wskazującą na nadrzędną pozycję wypowiadających się wobec osoby lub grupy ludzi; może polegać na doradzaniu, wskazywać na współczucie lub bycie zmartwionym . Przykładowe wyrażenia wskazujące na patronizowanie podane są w nawiasie (do roboty, za robotę, Wzięli by się do uczciwej pracy to nie mieliby czasu i ochoty na takie cyrki; nie mają przyszłości; dobre czasy rodzą słabych ludzi; gdzie są ich rodzice; współczuję rodzicom tej nieszczęśnicy; lepiej posprzątaliby śmieci; mają się za lepszych; mają się za elitę; mają innych za plebs; ja też miałem/miałam 20 lat; ja w ich wieku; czego ich uczą w szkole; nie zna geografii; Brakuje tylko smoków, elfów i krasnoludów w tej bajce; pracować im się nie chce; do żabki na kasę; zarób uczciwie; To są skutki wychowania bezstresowego; narzekają żyjąc w świecie dobrobytu; dobrobyt psuje; Jeśli młodzież się nie otrząśnie to czeka ich marny los; zalecam przymusowe leczenie; niech się weźmie do nauki; trzeba zmusić do roboty; Sami aktywisci tylko pracowac nie ma komu; rozpieszczeni; płatki śniegu; atencjusze; nie będziecie ich nagrywać to dzieci do szkół wrócą; do lasu drzewa sadzić; nudzą się; a mogliby zrobić coś naprawdę pożytecznego; Taty pasa na dupie nie było w dzieciństwie i to jest efekt; do wojska ich brać; )\n. Odpowiedz, podając wyłącznie jedną cyfrę 1, wtedy i tylko wtedy, jeżeli komentarz wyraża patronizowanie, zgodnie z podaną definicją. Odpowiedz 0, jeżeli tego nie wyraża. Jeżeli komentarz wyraża inne rodzaje mowy nienawiści, ale nie patronizowanie, odpowiedz 0. Nie udzielaj żadnych innych odpowiedzi poza 1 albo 0 ani nie uzasadniaj wykonanej klasyfikacji\n. Komentarz: "


qwen_patronizing <- classify_with_timing(
  df              = comments,
  text_col        = "text",
  new_col         = "patronizing",
  prompt_template = patronizing_prompt,
  model           = model_qwen
) # Done! 30574 comments in 20209.2s (0.7s per comment)

# save.image()

conspiracy_prompt <- "Jesteś klasyfikatorem, zawsze odpowiadasz wyłącznie jedną cyfrą: 1 albo 0. Zaklasyfikuj następujący komentarz zamieszczony pod wideo na YouTube jako wyrażający lub nie wyrażający teorie spiskowe, czyli wyjaśnianie sytuacji poprzez stwierdzenie istnienia tajemnej zmowy, w której uczestniczą potężne podmioty. Przykładowe wyrażenia wskazujące na teorie spiskowe podane są w nawiasie (dostają duże pieniądze, im płacą, za darmo nie siedzą, kto sponsoruje to testowanie w obecnej sytuacji geopolitycznej, chodzi nie o żadne idee ekologiczne lecz wyłącznie o testowanie służb i obywateli, bogaci sponsorują badania i ma wychodzić co wychodzi, to tylko eurobiznes, są opłacani przez wiadomo kogo, chore lewackie idee, doskonale zdają sobie sprawę z prawdziwych celów, o których trąbi Klaus Schwab; działają w imię leninowskiego komunizmu; wyprane mózgi i kasa; nie robią tego za darmo; kto ich sponsoruje; zmanipulowana hołota; ale oni tak sami z siebie?; są pożytecznymi idiotami dzięki którym elity mogą rządzić; manipulują; są zmanipulowani; wciągnięci do sekty; skutek uboczny 4 dawki szprycy; skutek uboczny szczepień; covid; histeria klimatyczna; zmiany klimatyczne były zawsze; doskonale przygotowana gierka; niech ujawni kto im płaci; fundacja Batorego; granty się zgadzają; zasłona dymna; dywersja; wszystko wyreżyserowane; komuś w kuluarach na tym zależy; Ci, którzy namieszali Ci w głowie latają odrzutowcami na lunch i golfa; to ustawka; jak z pandemią; $oro$; współpracują z wielkimi korporacjami przeciwko zwykłym ludziom; lobby dobrze płaci; zmiany klimatu nie są spowodowane działalnością człowieka)\n. Odpowiedz, podając wyłącznie jedną cyfrę 1, wtedy i tylko wtedy, jeżeli komentarz wyraża teorie spiskowe, zgodnie z podaną definicją. Odpowiedz 0, jeżeli tego nie wyraża. Jeżeli komentarz wyraża inne rodzaje mowy nienawiści, ale nie teorie spiskowe, odpowiedz 0. Nie udzielaj żadnych innych odpowiedzi poza 1 albo 0 ani nie uzasadniaj wykonanej klasyfikacji\n. Komentarz: "


political_prompt <- "Jesteś klasyfikatorem, zawsze odpowiadasz wyłącznie jedną cyfrą: 1 albo 0. Zaklasyfikuj następujący komentarz zamieszczony pod wideo na YouTube jako wyrażający lub nie wyrażający treści polityczne, czyli jakiekolwiek wzmianki o polityce, politykach i koncepcjach politycznych. Przykładowe wyrażenia wskazujące na treści polityczne podane są w nawiasie (wyborcy Tuska i tęczowego Rafałka, lewackie ścierwa, zdrajcy, sabotują nasz kraj, podopieczni Trzaskowskiego, komuniści, lewaki, dziadek Soros, opłacani przez zielonych i tęczowych, pod wpływem lobbystów, oplacani przez lewactwo, Ci ludzie mają dziwne komunistyczne tendencje do sterowania czyimś życiem, kolejna wypustka marksizmu, kolejna metoda wprowadzenia komunizmu, To może być zagrywka, która pozwoli Trzaskowskiemu czy Tuskowi zagrać wobec Warszawiaków rycerza nia białym koniu; WOŚP; Owsiak; przesiąknięci ideologią przez szkołę; robią bohaterów z koszernych; Żydzi; Ukraina; Policja udaje że nic się nie dzieje; TW Bolek; Hołownia musi ich przeszkolić; zielone łagry; Głosowałem na Grzegorza Brauna; lewiszcze; rozwiązania jak za PRL)\n. Odpowiedz, podając wyłącznie jedną cyfrę 1, wtedy i tylko wtedy, jeżeli komentarz wyraża treści polityczne, zgodnie z podaną definicją. Odpowiedz 0, jeżeli tego nie wyraża. Jeżeli komentarz wyraża inne rodzaje mowy nienawiści, ale nie treści polityczne, odpowiedz 0. Nie udzielaj żadnych innych odpowiedzi poza 1 albo 0 ani nie uzasadniaj wykonanej klasyfikacji\n. Komentarz: "

criminalization_prompt <- "Jesteś klasyfikatorem, zawsze odpowiadasz wyłącznie jedną cyfrą: 1 albo 0. Zaklasyfikuj następujący komentarz zamieszczony pod wideo na YouTube jako wyrażający lub nie wyrażający kryminalizację, czyli utożsamianie działań osoby lub grupy osób z działalnością przestępczą lub nielegalną, jak terroryzm i wandalizm. Przykładowe wyrażenia wskazujące na kryminalizację podane są w nawiasie (eko-terroryści, terroryści, grupa przestępcza, terroryzm, terroryzują miasto, akty terrorystyczne, ekopojeby, organizacja terrorystyczna, ekozjeby)\n. Odpowiedz, podając wyłącznie jedną cyfrę 1, wtedy i tylko wtedy, jeżeli komentarz wyraża kryminalizację, zgodnie z podaną definicją. Odpowiedz 0, jeżeli tego nie wyraża. Jeżeli komentarz wyraża inne rodzaje mowy nienawiści, ale nie kryminalizację, odpowiedz 0. Nie udzielaj żadnych innych odpowiedzi poza 1 albo 0 ani nie uzasadniaj wykonanej klasyfikacji\n. Komentarz: "

qwen_conspiracy <- classify_with_timing(
  df              = comments,
  text_col        = "text",
  new_col         = "conspiracy",
